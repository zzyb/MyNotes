package example.connector.source.custom;
 ...
 * 可重置的数据源
 */
public class StringWithTimeAndWatermark extends RichSourceFunction<String> implements CheckpointedFunction {
...
//                sourceContext.collect(result);
                // 发出记录 和与之关联的时间戳 !!!
                sourceContext.collectWithTimestamp(result, Long.parseLong(values[3]));
                // 发出水印
                sourceContext.emitWatermark(new Watermark(Long.parseLong(values[3])));
                // 观察kafka等数据源如何实现和使用该方法 !!!
                // 观察kafka等数据源如何实现和使用该方法 !!!
                // 观察kafka等数据源如何实现和使用该方法 !!!
                // 观察kafka等数据源如何实现和使用该方法 !!!
